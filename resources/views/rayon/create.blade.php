<x-layout>
    <x-slot:header>
        <h1 class="text-xl font-semibold text-blue-900">Tambah Data Rayon</h1>
        <div class="space-x-2">
            ...
            <a href="{{ route('rayon.index') }}" class="text-sm text-gray-400 hover:underline dark:text-blue-500">Data
                Rayon</a>
            <span class="text-sm text-gray-400 dark:text-gray-400">/</span>
            <span class="text-sm text-gray-600 dark:text-gray-400">Tambah Data Rayon</span>
        </div>
    </x-slot:header>
        ...
        <form action="{{ route('rayon.store') }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <input type="text" id="rayon" name="rayon" class="w-full border-gray-300 rounded"
                    placeholder="Rayon" value="{{ old('rayon') }}">
            ...
            <div class="mb-6">
                ...
                <select name="user_id" class="w-full border-gray-300 rounded select2">
                    ...
                    @foreach ($users as $item)
                        <option @selected(old('user_id') === "$item->id") value="{{ $item->id }}">{{ $item->name }}</option>
                    @endforeach
                </select>
            ...
            <button type="submit" class="px-4 py-2 text-white bg-blue-600 rounded hover:bg-blue-700">Tambah</button>
        </form>
    </div>
